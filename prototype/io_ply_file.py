...
def write_ply_file(filename, points):
    ...
    for point in points:
        file.write(str(point[0]) + " " + str(point[1]) + " " + str(point[2]) + " " + str(int(point[3]/100))+ " " + str(int(point[4]/100 % 256))+ " " + str(int(point[5]/100% 256))+ "\n")
    file.close()

def create_point_cloud(vertecies_filename, point_cloud_filename):
    ...
    write_ply_file(point_cloud_filename, house_vertecies[1:])####1??? RETURN LATER
