...
FILEPATH_FOR_3D_COORD = 'coordinates3D/coordinates3d_ideal_color_newTEST2.txt'
FILE = None
KP_FIRST_IMG = None
DES_FIRST_IMG = None
IMG_F_BGR = None
...
def step_processing(img_second_bgr, camera_move, ground_truth_r, ground_truth_t, delta_t, true_dist = 'FUCK DELETE THIS', border=False):
    ...
    # TMP_IMG = img_second
    # TMP_IMG = cv.drawKeypoints(img_second, kp2, TMP_IMG)
    ...
    # tmpM = sift_and_matches.draw_matches_img(matches, img_first, kp1, img_second, kp2)
   ...
   # mean_distance_for_filter = distance.distanceMean3DTesting(coord, MAX_DISTANCE)
    ...
    #     print(len(COORD_TMP), len(COLOR_TMP))
    #     if STEP% 1 == 0:
    #         write_3d_to_file_for_step_and_ply(COORD_TMP, ground_truth_r, ground_truth_t, COLOR_TMP, 'coordinates3D/stepsclouds/twoDots/4/coord3d/pointsStep_'+str(STEP)+'.txt','coordinates3D/stepsclouds/twoDots/4/ply/pointsStep_'+str(STEP)+'.ply' )
    #         COORD_TMP.clear()
    #         COLOR_TMP.clear()
    # STEP+=1
    saveTruthAndKalmanOnePoint(true_dist, filter_distance, ground_truth_t)
    return RFlag, LFlag, filter_distance
    ...
    if FILE is not None:
        if FILE.closed == False:
            for ind, i in enumerate(coord):
                tmpStr = str(i[0][0]) + ' ' + str(i[1][0]) + ' ' + str(i[2][0]) + ' ' + str(color[ind][2]) + ' ' + str(
...
def initAlgorithm(img_first, filepath3d = ''):
    global KP_FIRST_IMG, DES_FIRST_IMG, IMG_F_GRAY, IMG_F_BGR, FILEPATH_FOR_3D_COORD, STEP
    STEP = 0
    IMG_F_BGR = img_first
    if filepath3d != '':
